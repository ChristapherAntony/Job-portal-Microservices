export const VIEW_APPLICATOIONS = (id) => `applications/${id}`;
export const VIEW_APPLIED_CANDIDATE_DETAILS = (candidateId, applicationId) => `/recruiter/jobs/applications/candidate/${candidateId}/${applicationId}`;

export const VIEW_SKILLTEST_TABLE = `/recruiter/skill-test`;
export const VIEW_SKILLTEST_DETAILS = (id) => `/recruiter/skill-test/details/${id}`;
export const VIEW_SKILLTEST_ADD = `/recruiter/add-skill-test`;


export const APPLIED_JOBS = '/candidate/my-jobs';
export const APPLIED_JOBS_DETAILS = (id) => `/candidate/my-jobs/${id}`;


export const getTestQuestions = (testId, recruiterId) => `/candidate/take-test` //delete  


export const TEST_COMPLETED = (applicationId) => `/candidate/skill-test/completed/${applicationId}` //delete  
